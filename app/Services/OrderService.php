...
class OrderService extends BaseService
{
    ...
    static public function payNotifySuccess($orderSn)
    {
        $order = Order::withTrashed()->with(['product'=>function($query){
            $query->select(['id','send_sms_or_not','booking_information']);
        ...
        if ($result){
            //检查是否发放预约短信
            if ($order->product->send_sms_or_not == Order::SEND_SMS_1 && $order->product->booking_information){
                // 发送短信
                $send = SmsService::sendSms($order->tel, 'booking', $order->product->booking_information);
