...
{   
    ...
    public function distribution($config)
    {
        ...
        ! $fUid || ! $fMoney ?: AccountService::addMoney($fUid, $fMoney, UserAccountRecord::OBJECT_TYPE_1, 0, $this->order->id);
        setLog('订单分销:一级分销', $startTime, ['uid'=>$fUid, 'money'=>$fMoney]);
        
        // 二级分销加钱
        $sUid = $config['secondary']['uid'];
        $sMoney = $config['secondary']['money'];
        ! $sUid || ! $sMoney ?: AccountService::addMoney($sUid, $sMoney, UserAccountRecord::OBJECT_TYPE_6, 0, $this->order->id);
        setLog('订单分销:二级分销', $startTime, ['uid'=>$sUid, 'money'=>$sMoney]);
        
        // 团队分销加钱
        $teamUid = $config['team']['uid'];
        $teamMoney = $config['team']['money'];
        ! $teamUid || ! $teamMoney ?: AccountService::addMoney($teamUid, $teamMoney, UserAccountRecord::OBJECT_TYPE_2, 0, $this->order->id);
