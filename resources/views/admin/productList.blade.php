				...
				<tbody>
					<tr v-for="(n,index) in list">
						...
						<td>
							...
							<button class="layui-btn layui-btn-sm layui-btn-normal" @click="sold_out(n.id)" v-if="n.status!=2&&n.status!=3">下架</button>
							<button class="layui-btn layui-btn-sm layui-btn-normal" @click="putaway(n.id)"  v-if="n.status==2||n.status==3">上架</button>
							<button class="layui-btn layui-btn-sm layui-btn-danger" @click="conceal(n.id)" v-if="n.status!=3">隐藏</button>
						</td>
					</tr>
				</tbody>
			</table>
