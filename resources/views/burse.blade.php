    ...
    <div class="bs3">
        <ul>
            <li v-for="d in data.data" v-cloak>
                <div class="ut41">
                    <p>订单号码：@{{d.order_info.sn}}</p>
                    <p><a style="text-decoration: underline;" :href="details(d.product.id)">@{{d.product.subtitle}}</a></p>
                </div>
                <div class="ut41">
                    <p class="">用户姓名：@{{d.order_info.name}}</p>
                    <p class="">联系电话：@{{d.order_info.tel}}</p>
                    <p class="ut411">佣金：¥@{{d.money}}</p>
                    <p v-if="d.order_info.status==0">当前状态：未支付</p>
                    <p v-if="d.order_info.status==1">当前状态：已支付</p>
                    <p v-if="d.order_info.status==2">当前状态：已预约</p>
                    <p v-if="d.order_info.status==3">当前状态：已发货</p>
                    <p v-if="d.order_info.status==4">当前状态：已完成</p>
                </div>
            </li>
